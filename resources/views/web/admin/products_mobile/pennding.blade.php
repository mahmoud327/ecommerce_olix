        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                   ...
                   <a  href="#"
                        data-toggle="modal" data-target="#available_all"><i
                        class="btn btn-primary  btn-sm" id="btn_available_all"  >available all </i>&nbsp;&nbsp;
                   </a>



                   <a  href="#"
                        data-toggle="modal" data-target="#disapprove_all"><i
                        class="btn btn-primary  btn-sm" id="btn_disapprove_all"  >disapprove all  </i>&nbsp;&nbsp;
                   </a>
                   <a  href="{{route('product.trash_products_of_pennding')}}" class="btn btn-danger btn-sm">trash all</a>


                </div>

                <input type="hidden" value="{{env('AWS_S3_URL')}}" id="env">
          ...
          <!-- ////////////////////////modela for delete///////////// -->
                    ...
                    <div class="modal fade" id="available_all" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                       aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">available_all</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                    <form action="{{route('products.available_all') }}" method="post">
                                        {{ csrf_field() }}
                                </div>
                                <div class="modal-body">
                                    <input class="text" type="hidden" id="available_all_id" name="available_all_id" value=''>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ trans('lang.close') }}</button>
                                    <button type="submit" class="btn btn-danger">{{ trans('lang.save') }}</button>
                    ...
                    <div class="modal fade" id="disapprove_all" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                       aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">disapprove_all</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                    <form action="{{route('products.disapprove_all') }}" method="post">
                                        {{ csrf_field() }}
                                </div>
                                <div class="modal-body">
                                    <input class="text" type="hidden" id="disapprove_all_id" name="disapprove_all_id" value=''>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ trans('lang.close') }}</button>
                                    <button type="submit" class="btn btn-danger">{{ trans('lang.save') }}</button>
                        ...
                        <table id="example1" class="table key-buttons text-md-nowrap" data-page-length='50'style="text-align: center">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-bottom-0">Mobile_user </th>
                                    <th class="border-bottom-0">email_user </th>
                            ...
                            <tbody>
                                ...
                                @foreach ($products as $product)
                                        ...
                                        <td>{{ $product->quantity}}</td>
                                            ...
                                            <td>
                                                   <a href="{{ route('product.approve' , $product->id) }}"
                                                    ...
                                                    <a  href="#" data-product_id="" class="btn btn-danger btn-sm"
                                                        data-toggle="modal" data-target="#disapprove{{$product->id}}"><i
                                                      >disapprove</i>
                                                    </a>
                                    ...
                                    $filters = \App\Models\FilterRecurring::orderby('position', 'asc')->whereIn('id', $filtersIds)->with(array('subFiltersRecurring' => function($q) use ($subFiltersIds)
                                    {
                                    ...
                                    <div class="modal fade" id="disapprove{{$product->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                                        aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <form action="{{route('product.disapprove',$product->id)}}" method="post" enctype="multipart/form-data">
                                                        {{ csrf_field()}}
                                                        <div class="form-group">
                                                            <textarea   class="form-control form-control-sm mg-b-20" data-parsley-class-handler="#lnWrapper"
                                                            name="rejected_reason" required placeholder="reject reason"></textarea>
                                                        </div>
              ...
              if(data.status == 1)
              {
                 ...
                 $("#example1 tbody").children().remove();
                    $.each(data.data, function(index,child){
                        $("#example1 tbody").append('<tr  class="tr" data-s3='+env+'>'+

                        '<td>'+
                            '<input id="cat-box" type="checkbox" name="categories"  value='+child.id+' class="box1" >'+
                        '</td>'+
                        '<td>'+ i++ +'</td>'+
                        '<td>'+child.name.en+'</td>'+
                        '<td>'+child.category.name.en+'</td>'+
                        '<td>'+
                           '<a  href="users/">'+child.user.mobile+'</a>'+
                        '</td>'+
                        '<td>'+
                           '<a href="users/">'+child.user.email+'</a>'+
                        '</td>'+
                        '<td>'+child.price+'</td>'+
                        '<td>'+child.quantity+'</td>'+
                        '<td>'+child.name.en+'</td>'+
                        '<td>'+
                            '<a  href="product/approve'+child.id+' " class="btn btn-primary btn-sm"> approve </a>'+

                             '<a  href="#" data-product_id="" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#disapprove'+child.id+' "> <i>disapprove</i>'+
                             '</a>'+

                            '<div class="modal fade" id="disapprove'+child.id+'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"  aria-hidden="true">'+
                                        '<div class="modal-dialog" role="document">'+
                                            '<div class="modal-content">'+
                                                '<div class="modal-header">'+
                                                    '<h5 class="modal-title" id="exampleModalLabel">reason rejected</h5>'+
                                                ...
                                                '<form action="product/disapprove'+child.id+' " method="post" enctype="multipart/form-data">'+

                                                        '<div class="form-group">'+
                                                            '<textarea   class="form-control form-control-sm mg-b-20" data-parsley-class-handler="#lnWrapper" name="rejected_reason" required placeholder="reject reason">'+
                                                            '</textarea>'+
                                                        '</div>'+
                        ...
                        '<td>'+
                            ...
                            '<a class="modal-effect btn btn-sm btn-info" data-effect="effect-scale" data-toggle="modal" href="#exampleModal2" data-id='+child.id+' title="تعديل" >'+
                             '<i class="fa fa-eye">'+
                                 '</i>'+
                              '</a>'+
...
<script>
    $(function()
    {
        $("#btn_available_all").click(function() {
            var selected = new Array();
            $("#example1 input[type=checkbox]:checked").each(function() {
                selected.push(this.value);
            ...
            if (selected.length > 0) {
                $('#available_all').modal('show')
                $('input[id="available_all_id"]').val(selected);
...
<script>
    $(function()
    {
        $("#btn_disapprove_all").click(function() {
            var selected = new Array();
            $("#example1 input[type=checkbox]:checked").each(function() {
                selected.push(this.value);
            ...
            if (selected.length > 0) {
                $('#disapprove_all').modal('show')
                $('input[id="disapprove_all_id"]').val(selected);
