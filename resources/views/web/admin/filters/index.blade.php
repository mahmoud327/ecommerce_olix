...
@section('title')
    {{ trans('lang.page_title_of_filter') }}
@stop
...
<div class="row">
    ...
    <div class="col-xl-12">
        <div class="card mg-b-20">
                ...
                <div class="d-flex justify-content-between">
                    @can('create_filter')
                     {{--    <a class="modal-effect btn btn-outline-primary" data-effect="effect-scale"
                            data-toggle="modal" href="#modaldemo8">{{ trans('lang.add_filter') }}</a> --}}
                    @endcan 


                    {{-- <a href="{{route('filters.trash_filter', $category_id)}}" class="btn btn-sm btn-danger">
                        Trash
                    </a> --}}
                </div>
            ...
            <div class="card-body">
                    ...
                    <table id="example1" class="table key-buttons text-md-nowrap" data-page-length='50'
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="border-bottom-0">{{ trans('lang.filter_name') }}</th>
                                <th class="border-bottom-0">image</th>
                                <th class="border-bottom-0">code</th>
                                <th class="border-bottom-0">PATH</th>
                        ...
                        <tbody>
                            @foreach ($filters as $filter )
                                ...
                                <tr>
                                    ...
                                    <td>{{ $filter->name }}</td>
                                    <td><img style="width: 80px;height:60px" src="{{env('AWS_S3_URL').'/uploads/Filters/'.$filter->image}}" alt="categories-image"></td>

                                    <td>{{$filter->id}}</td>
                                    <td>  <a href="{{route('recurring_filter.index' ,$filter->filter_type_id)}}">{{ $filter->name }}</a> </td>

                                    
                                    <td>
                                        {{-- @can('sub_filter') --}}
                                            <a href="{{route('sub_filters.index' , $filter->id)}}" class="btn btn-sm btn-primary">
                ...
                {{-- 
                                @include('web.admin.filters.delete_modal', ['filter' => $filter])
                                @include('web.admin.filters.edit_modal', ['filter' => $filter, 'category_id'   => $category_id]) --}}
    ...
    {{-- @include('web.admin.filters.add_modal', ['category_id'   => $category_id]) --}}
...
<script>
    ...
    $(function (){
        ...
        $('.edit input#sub_account:checked').parent().css("display", "block").siblings().css("display", "block");
        $('.edit input#sub_account:checked').parent().parent().siblings('i').removeClass('si-arrow-down-circle').addClass('si-arrow-right-circle');
    })
...
<script>
   ...
   Dropzone.options.dpzMultipleFiles = {
       ...
       ,
       url: "{{ route('admin.filters.images.store') }}", // Set the url
       success:
