...
<div class="row">
            ...
            <div class="card-header pb-0">
                <div>
                         ...
                         {{-- @endcan --}}
                          ...
                          {{-- @can('all_sub_categories') --}}
                           @if($subCategories->count())

                            <input type="hidden" value="{{$subCategories->first()->parent_id}}"  id="parent_id"/>
                            @else
                          ...
                          @endif
                            ...
                            <a href="{{route('toggle_all', $subCategories->first()->parent_id )}}" id="toggleAll" type="button" class="btn btn-danger btn-sm"  > <i class="fas fa-times"></i> all </a>

                            @else
                            <a href="{{route('toggle_all', $subCategories->first()->parent_id)}}" id="toggleAll" type="button" class="btn btn-primary btn-sm" > <i class="fas fa-check"></i> all</a>
                            @endif
                   ...
                   @can('delete_sub_categories')
                        ...
                        <a href="{{route('category.trash_catrgory' ,  $category->id)}}" class="btn btn-sm btn-danger">
                            Trash
                        </a>
                   @endcan
                   @can('create_advertisments')

                        @if($parent)
                        <a  href="{{route('advertisments.index',$parent->id)}}" ><i
                            class="btn btn-primary btn-sm"> add advertisments</i>&nbsp;&nbsp;
                         </a>
                        @endif
                   @endcan
                    ...
                    <div class="text-center"style="display: inline; margin-left:20%;">
                        ...
                        <i class="fa fa-angle-double-left"> </i>
                            ...
                            @foreach ($category->getParentsNames()->reverse() as $item)
                                ...
                                <a  href="{{route('category.subCategories' , $item->id)}}" > {{ $item->name }}</a>
             ...
             @if($subCategories->count())
                  ...
                  <form class="form-inline" action="{{ route('search_subCategories') }}" method="GET">
                      ...
                      <div class="form-group mx-sm-3">
                        ...
                        <input type="hidden" name="parent_id" value="{{$subCategories->first()->parent_id}}"/>
    ...
    @include('web.admin.categories.subCategories.add_modal')
...
<script>
    $(document).ready(function(){
     ...
     {
      $.ajax({
       url:"pagination/fetch_data?page="+page+"&sortby="+sort_by+"&sorttype="+sort_type+"&parent_id="+parent_id,
