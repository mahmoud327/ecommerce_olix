...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto">edit advertisment</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0"> /
                edit  advertisment</span>
        </div>
    </div>
</div>
<!-- breadcrumb -->
        ...
        <div class="card">
            <div class="card-body">
                    ...
                    <div class="pull-right">
                        @if($advertisment->category_id==0)
                          <a class="btn btn-primary btn-sm" href="{{route('advertisments.index',0)}}">{{ trans('lang.back') }}</a>
                        @else
                        <a class="btn btn-primary btn-sm" href="{{route('advertisments.index',$advertisment->category_id)}}">{{ trans('lang.back') }}</a>
                        @endif
                    </div>

                    <br>

                    <form action="{{route('advertisments.update',$advertisment->id)}}" method="post" enctype="multipart/form-data">
                      @method('put')
                        @csrf
                            ...
                            @endif --}}
                            <div class="" id="div_link" @if($advertisment->link == '') style="display:none" @endif>
                                <input  type="url"  class="form-control" name="link" id="link" value="{{$advertisment->link}}"  style="width:32%" placeholder="link">
                            </div>

                            <br>
                            <div class="" id="div_type" @if($advertisment->type_id==null) style="display:none" @endif>
                                <select class="form-control" id="type"  name="type" aria-label="Default select example" style="width:32%">
                                    <option value='no_select'>  select type  </option>
                                    <option value="product" {{$advertisment->type=="product" ? 'selected' :''}}>  product  </option>
                                    <option value="organization" {{$advertisment->type=="organization" ? 'selected' :''}}>organization </option>
                                  </select>
                             </div>
                             ...
                             <div class="col-sm-2"  @if($advertisment->type=="product") style="display:inline" @else style="display:none" @endif id="products">

                                <select  name="product_id" class="products" style="width:32%">

                                    <option value="">  select product  </option>
                                    @foreach ($products->get() as $product)
                                            <option  {{ ($product->id == $advertisment->type_id)  ?'selected' :'' }}  value="{{$product->id}}">
                                                {{$product->name}}
                                            </option>
                                    @endforeach
                                </select>
                            ...
                            @inject('organizations','App\Models\Organization')
                             <div class="col-sm-2"  @if($advertisment->type=="organization") style="display:inline" @else style="display:none" @endif  id="organizations">
                                
                                <select  name="organization_id" class="organizations" style="width:32%">
                                    <option value="" >  select organization  </option>
                                    @foreach ($organizations->get() as $organization)
                                   
                                            <option  {{ ($organization->id ==$advertisment->type_id)  ?'selected' :'' }} value="{{$organization->id}}">
                                                {{$organization->name}}
                                            </option>
                                    @endforeach
                                </select>
...
<script>
   ...
   Dropzone.options.dpzMultipleFiles = {
     ...
     removedfile:function(file)
 	{
        ...
        //  console.log(imgSrcValue);
 		...
 		$.ajax({
 			...
 			url: "{{ URL::to('admin/delete/advertisment_image') }}",
             type: "GET",
            dataType: "json",
     ...
     init:function(
            ...
            var mock = {name: '{{URL::to('/').'/uploads/advertisments/'.$advertisment->image }}'};
            this.emit('addedfile',mock);
            this.options.thumbnail.call(this,mock,'{{ url(env('AWS_S3_URL').'/uploads/advertisments/'.$advertisment->image) }}');
