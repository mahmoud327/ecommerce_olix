            ...
            <div class="card-header pb-0">
                ...
                <div class="">
                    @can('create_advertisments')
                        ...
                        href="{{route('advertisments.create',$category_id)}}">Add advertisment </a>
                    @endcan
                  
                   
                 </div>
                <br>
              @can('delete_advertisments')
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <form action="{{route('advertisments.delete_all') }}" method="post">
                                    {{ method_field('delete') }}
                                    {{ csrf_field() }}
                            </div>
                    ...
                    <table id="example1" class="table key-buttons text-md-nowrap" data-page-length='50'
                        ...
                        <tbody>
                            @foreach ($advertisments as $advertisment )
                                ...
                                <tr>
                                    <td><input id="cat-box" type="checkbox" name="admins"  value="{{$advertisment->id}}" class="box1" ></td>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $advertisment->link }}</td>
                                    <td><img style="width: 80px;height:60px" src="{{env('AWS_S3_URL').'/'.'uploads/advertisments'}}/{{$advertisment->image}}" alt="advertisment-image"></td>


                                    <td>
                                         @can('update_advertisments')
                                         <a class="btn btn-sm btn-info"
                                         href="{{route('advertisments.edit',$advertisment->id)}}" title="edit"><i class="las la-pen"></i></a> 
                                         @endcan
                                         
                                         
                                         @can('delete_advertisments')
                                            <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale"
                                            data-toggle="modal" href="#delete_advertisment{{ $advertisment->id }}" title="delete"><i
                                            class="las la-trash"></i></a>
                                         @endcan

                                        @include('web.admin.advertisments.modal_delete' ,['advertisment'=>$advertisment])
